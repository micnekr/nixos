...
-- Prompt if trying to exit file without saving, as opposed to failing
vim.opt.confirm = true

-- Spell stuff
vim.opt.spell = true
vim.opt.spelllang = "en_gb"
-- local spell_file_path = vim.fn.stdpath("config") .. "/en.utf-8.add"
local spell_file_path = "~/.config/nixos/modules/nvim/en.utf-8.add"
-- Save personal dictionary in the file
vim.opt.spellfile = spell_file_path
-- Regenerate spellfile if needed (e.g. if .spl is not present because it is gitignored)
if vim.fn.filereadable(spell_file_path) then
	vim.cmd({
		cmd = "mkspell",
		args = { spell_file_path .. ".spl", spell_file_path },
		bang = true,
		mods = { silent = true },
	})
end
...
vim.keymap.set({ "n", "v" }, "<leader>R", '"_', { desc = "Put into black hole buffer" })
vim.keymap.set({ "n", "v" }, "<leader>r", '"0', { desc = "Take from yank buffer" })

-- Save file(s)
vim.keymap.set("n", "<Space>S", "<cmd>w<CR>")
vim.keymap.set("n", "<Space><C-S>", "<cmd>wa<CR>")
...
--[===[ This is the non-nix way of doing this
...
require("nixCatsUtils.lazyCat").setup(nixCats.pawsible({ "allPlugins", "start", "lazy.nvim" }), {
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		dependencies = {
			...
			-- Sets the default select ui to telescope
		...
		config = function()
			...
			require("telescope").setup({
				...
				--
				defaults = {
					-- When showing long paths, show the end
					path_display = { "truncate" },
					-- TODO: this looks useful, try this out
					-- mappings = {
			...
			vim.keymap.set("n", "<leader>sd", "<CMD>Oil<CR>", { desc = "[S]earch [D]irectory" })
			vim.keymap.set("n", "<leader>s.", builtin.oldfiles, { desc = '[S]earch Recent Files ("." for repeat)' })
			vim.keymap.set("n", "<leader><leader>", builtin.buffers, { desc = "[ ] Find existing buffers" })
			vim.keymap.set("n", "<leader>sf", builtin.find_files, { desc = "[S]earch [F]iles" })
			vim.keymap.set("n", "<leader>sf", builtin.find_files, { desc = "[S]earch [F]iles" })
			vim.keymap.set("n", "<leader>sF", function()
				builtin.find_files({ hidden = true, no_ignore = true, path_display = { "truncate" } })
			end, { desc = "[S]earch [F]iles without ignoring" })
	...
	{ -- LSP Configuration & Plugins
		...
		config = function()
			...
			-- TODO: find nix alternative
			-- -- TODO: Run PylspInstall to install relevant 3rd party packages, such as pylsp-mypy
		...
		config = function()
			...
			cmp.setup({
				...
				mapping = cmp.mapping.preset.insert({
					...
					-- TODO: see what this is all about, and fix the keybindings
					-- TODO: look at what snippets are available, what snippets to add.
	...
	},
	{
		...
		config = function()
			...
			ng.setup({})
			vim.keymap.set("n", "<leader>g", ng.open)
		end,
	},
	{
		"ggandor/leap.nvim",
		priority = 1001, -- load before the colorscheme so that colors are applied properly
		init = function()
			...
			-- Do not jump to first found entry
			leap.opts.safe_labels = {}

			leap.opts.labels = "tnseriaoplufdhvkwyc,q;z.'TNSERIAOPLUFDHVKWYC<Q:Z>"
	...
	-- { -- e.g. shows function declaration when in its definition
	...
	-- 	-- Or maybe https://github.com/RRethy/nvim-treesitter-textsubjects/ instead
